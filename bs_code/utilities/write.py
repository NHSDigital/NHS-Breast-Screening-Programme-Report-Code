...
def add_footnote_refs(sheetname, start_cell, org_col_letter,
    ...
    # Where there is no region to be checked, then the only the org value will
    # be used
    for row in range(row_start, row_stop):
        ...
        values_to_update = values_to_update[(values_to_update.isin([check_org])
                                            ...
                                            (values_to_update.isin([check_region])
                                             .any(axis=1))]
        # If they are present then add the superscripted foootnote ref
        if values_to_update.empty:
            pass
        else:
            # Extract the footnote reference value and check it's length
            ...
            # Use the original string length to check the foonote ref start position
...
def select_write_type(df, write_type, output_path, output_name,
    ...
    # If write_type is csv, then write the putput to a csv
    if write_type == "csv":
